...
private var start:Transform;					// Stores the Start transform that is positioned at the Start Target (good place to put particle systems and similar | no warning if missing)
private var end:Transform;						// Stores the End transform that is positioned at the End Target
var vertexCount:float;							// For monitoring vertex count (this is not a editable value)
private var lineRenderer:LineRenderer;			// LineRenderer attached to Beam gameObject
private var runSpiral:boolean = true;
private var lineRendererMat:Material;			
...
function Update () {
	if(runSpiral){
		...
		var distance:float = Vector3.Distance(targetStart.position,targetEnd.position);	
		if(smooth)
			vertexCount = LenghtOfLineRenderer*distance*lenghtResolutionMultiplier;
		else
			vertexCount = (LenghtOfLineRenderer+.1);
		if(vertexCount<15)
			vertexCount =15;
		...
		for(var i :int = 0; i < vertexCount-1; i++){
			...
			var rr:float =Mathf.PerlinNoise((i*1.0)/distance*noiseScale*n,Time.time*noiseSpeed)*noise+1;
			if(sineHeightFrequency!=0)
				var ry:float = Mathf.Sin(sineHeightFrequency*pos+(Time.time*sineHeightSpeed))*sineHeight*grow*rr;
			if(sineDepthFrequency!=0)
				var rx:float = Mathf.Cos(sineDepthFrequency*pos+(Time.time*sineDepthSpeed))*sineDepth*grow*rr;
			if(sineLenghtFrequency!=0)
				var rz:float = Mathf.Sin(sineLenghtFrequency*pos+(Time.time*sineLenghtSpeed))*sineLenght*grow*rr;
	...
	transform.LookAt(targetEnd, transform.up);
